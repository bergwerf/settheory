...
Definition D (X : P C) α := ∀m, ∃β, α ≠ β /\ (Eqn m α ∩ X) β.
...
eapply eqn_trans. apply Hm. eapply eqn_incl. 2: apply H1β. auto. easy.
Qed.
...
pose(F1 m α i b :=
  (Eqn m α ∩ X = ∅ /\ b = false) \/
  (Eqn m α ∩ X ≠ ∅ /\ ∃β, MinPre (Eqn m α ∩ X) (S i) β /\ b = β i)).
...
assert(F3_func : ∀p, ∃!b, F3 (fst p) (snd p) b). {
  ...
  destruct (min_pre_ex (Eqn m α ∩ X) (S i)) as [β Hβ]. easy.
  exists (β i); split. now exists β. intros b [γ [Hγ R]]; subst b.
  eapply min_pre_unique. apply Hβ. apply Hγ. lia. }
...
- (* α ≠ zeros is not a limit point. *)
  ...
  (* Find β in (D ↑ n) (Shifts X) s.t. Eqn (S i) α β. *)
