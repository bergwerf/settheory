...
(* V is a subset of W. *)
Definition Inclusion {X} (V W : P X) := ∀x, V x -> W x.
...
Definition ProperSuperset {X} (V W : P X) :=
  Inclusion W V /\ ~Empty (exclude V W).
...
Lemma Inclusion_refl {X} (V : P X) : V ⊆ V.
Proof. easy. Qed.
...
Proof. intros. subst. split; apply Inclusion_refl. Qed.
